...
def ajouter_appareil():
    ...
    if len(execution) == 0:
        print(
            "Il n'y a pas aucune personne enregistrée dans la base de données, veuillez en ajouter une d'abord pour pouvoir affecter un propriétaire à l'appareil")
        return
    db.afficher_resultats(execution)
...
def ajouter_modeleAppareil():
    ...
    while not (const.entreeEstEntier(typeAppareil) and int(typeAppareil) >= 0 and int(typeAppareil) < len(type)):
        ...
        typeAppareil = input("Entrez le type d'appareil : ")
    commande = "INSERT INTO ModeleAppareils ('marque_appareil', 'modele_appareil', 'ram_appareil', 'processeur_appareil', 'type_appareil') VALUES ('" + marque + "', '" + modele + "', " + ram + ", '" + processeur + "', '" + \
               type[int(typeAppareil)] + "');"
    try:
        db.executer_commande_sql(commande)
    except Exception as e:
        print("Erreur lors de l'insertion de l'appareil :", e)
